...
BEGIN_DMXELEMENT_UNPACK( CParticleSystemDefinition ) 
	...
	DMXELEMENT_UNPACK_FIELD_UTLSTRING_USERDATA( "material", "vgui/white", m_pszMaterialName, "vmtPicker" )
	...
	DMXELEMENT_UNPACK_FIELD_UTLSTRING( "cull_replacement_definition", "", m_pszCullReplacementName )
	DMXELEMENT_UNPACK_FIELD_UTLSTRING( "fallback replacement definition", "", m_pszFallbackReplacementName )
	...
	for ( int i = 0; i < nCount; ++i )
	{
		...
		if ( !bFound )
		{
			if ( flist.Count() )							// don't warn if no ops of that type defined (server)
				Warning( (tchar*)"Didn't find particle function %s\n", pOpName );
		}
	}
}
...
CParticleSystemDefinition *CParticleSystemDefinition::GetFallbackReplacementDefinition() const
{
	if ( HasFallback() )
	{
		if ( !m_pFallback() )
		{
			const_cast< CParticleSystemDefinition* >( this )->m_pFallback.Set( g_pParticleSystemMgr->FindParticleSystem(m_pszFallbackReplacementName) );
		}
		return m_pFallback();
	}
	return NULL;
...
bool CParticleCollection::Init( CParticleSystemDefinition *pDef )
{
	...
	{
		Warning( (tchar*)"Particlelib: Missing precache for particle system type \"%s\"!\n", pDef ? pDef->GetName() : "unknown" );
...
bool CParticleCollection::Init( const char *pParticleSystemName )
{
	...
	if ( !pDef )
	{
		Warning( (tchar*)"Attempted to create unknown particle system type \"%s\"!\n", pParticleSystemName );
		return false;
	}
...
//-----------------------------------------------------------------------------
void CParticleCollection::Render(IMatRenderContext *pRenderContext, const Vector4D &vecDiffuseModulation, bool bTranslucentOnly, void *pCameraObject )
{
	if ( !IsValid() )
		return;
	...
	if ( !m_Sheet() )
	{
		m_Sheet.Set( g_pParticleSystemMgr->FindOrLoadSheet( m_pDef ) );
	}
			...
			for( int i = 0; i < nCount; i++ )
			{
				...
				if ( m_pDef->IsScreenSpaceEffect() )
				{
					...
					m_pDef->m_Renderers[i]->Render(
						pRenderContext, this, vecActualModulation, m_pOperatorContextData + m_pDef->m_nRenderersCtxOffsets[i] );
					pRenderContext->MatrixMode( MATERIAL_VIEW );
	...
	for( CParticleCollection *p = m_Children.m_pHead; p; p = p->m_pNext )
	{
		p->Render(pRenderContext, vecDiffuseModulation, bTranslucentOnly, pCameraObject );
	}
...
bool CParticleSystemMgr::Init( IParticleSystemQuery *pQuery, bool bAllowPrecache )
{
	...
	{
		Msg( (tchar*)"CParticleSystemMgr compiled using an old IMaterialSystem\n" );
		return false;
	}
...
#define INITPARTICLE_ATTRIBUTE( name )												\
	...
	for ( int i = 0; i < MAX_PARTICLE_ATTRIBUTES; i++ )
	{
		if ( !s_AttributeTable[ i ].pName )
		{
			...
			Warning( (tchar*)"CParticleSystemMgr::InitAttributeTable has an out-of-date attribute list! (element %d not set up)\n", i );
			Assert( 0 );
		}
	}
}
...
void CParticleSystemMgr::PrecacheParticleSystem( int nStringNumber, const char *pName )
{
	...
	{
		Warning( (tchar*)"Attempted to precache unknown particle system \"%s\"!\n", pName );
...
bool CParticleSystemMgr::ReadParticleDefinitions( CUtlBuffer &buf, const char *pFileName, bool bPrecache, bool bDecommitTempMemory )
{
	...
	{
		Warning( (tchar*)"Unable to read particle definition %s! UtlBuffer is probably the wrong type!\n", pFileName );
		return false;
	}
...
//	char pFallbackBuf[MAX_PATH];
	...
	Warning( (tchar*)"Particles: Missing '%s'\n", pFileName );
	return false;
}
...
CParticleCollection *CParticleSystemMgr::CreateParticleCollection( ParticleSystemHandle_t particleSystemName, float flDelay, int nRandomSeed )
{
	...
	if ( !pDef )
	{
		Warning( (tchar*)"Attempted to create unknown particle system with unknown symbol\n" );
		return NULL;
	}
	CParticleCollection *pParticleCollection = new CParticleCollection;
	pParticleCollection->Init( pDef, flDelay, nRandomSeed );
...
CParticleCollection *CParticleSystemMgr::CreateParticleCollection( const DmObjectId_t &id, float flDelay, int nRandomSeed )
{
	...
	if ( !pDef )
	{
		...
		Warning( (tchar*)"Attempted to create unknown particle system id %s\n", pBuf );
		return NULL;
	}
	CParticleCollection *pParticleCollection = new CParticleCollection;
	pParticleCollection->Init( pDef, flDelay, nRandomSeed );
...
CSheet *CParticleSystemMgr::FindOrLoadSheet( CParticleSystemDefinition *pDef )
{
...
void CParticleSystemMgr::DumpParticleList( const char *pNameSubstring /* = NULL */ )
{
	if ( pNameSubstring )
	{
		Msg( (tchar*)"New Particle Systems Matching '%s':\n", pNameSubstring );
	}
	else
	{
		Msg( (tchar*)"All Particle Systems:\n" );
	}
...
void GetParticleManifest( CUtlVector<CUtlString>& list, const char *pFile )
{
	...
	if ( manifest->LoadFromFileEX( g_pFullFileSystem, pFile, "GAME" ) )
	{
		for ( KeyValues *sub = manifest->GetFirstSubKey(); sub != NULL; sub = sub->GetNextKey() )
		{
			...
			Warning( (tchar*)"CParticleMgr::Init:  Manifest '%s' with bogus file type '%s', expecting 'file'\n", pFile, sub->GetName() );
		}
	}
	else
	{
		Warning( (tchar*)"PARTICLE SYSTEM: Unable to load manifest file '%s'\n", pFile );
	}
